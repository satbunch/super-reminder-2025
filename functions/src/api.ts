...
export const lineWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    if (!channelAccessToken || !channelSecret) {
      throw new Error(
        "LINE_CHANEL_ACCESS_TOKEN and LINE_CHANEL_SECRET must be set",
      );
    }
          ...
          events.map(async (event: WebhookEvent) => {
            ...
            if (message === "キャンセル") {
              ...
              await client.replyMessage({
                replyToken: event.replyToken,
                messages: [
                  { type: "text", text: "リマインドの登録をキャンセルしたよ" },
                ],
              });
              return;
            }
            ...
            if (session.status === "idle" && message === "リマインド一覧") {
              ...
              const flex = generateReminderFlex(
                reminderList,
            ...
            if (session.status === "waiting_for_time" && session.task) {
              ...
              // JST形式で表示
              const remindDateJST = new Date(
                remindAt.getTime() + 9 * 60 * 60 * 1000,
              );
            ...
            const randomMessage =
              exMessages[Math.floor(Math.random() * exMessages.length)];
